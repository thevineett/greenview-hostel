...
function Settings() {
  ...
  const changePassword = async (e) => {
    ...
    if (data.success) { toast.success("Password updated!", { position:"top-right", autoClose:2000, theme:"light" }); setTimeout(() => navigate("/student-dashboard"), 1500); }
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-0.5">Your profile and account settings</p>
      </div>

      <div className="bg-white rounded-2xl border border-green-100 shadow-sm p-6">
        <h2 className="text-base font-semibold text-gray-800 mb-4">Profile</h2>
        <div className="flex items-center gap-4 mb-5">
          <div className="w-14 h-14 rounded-2xl bg-green-500 text-white text-2xl font-bold flex items-center justify-center shadow-lg shadow-green-200">{student?.name?.charAt(0)}</div>
          <div>
            <p className="font-bold text-gray-800 text-lg">{student?.name}</p>
            <p className="text-sm text-gray-500">{student?.email}</p>
            <span className="text-xs bg-green-100 text-green-700 font-semibold px-2 py-0.5 rounded-full mt-1 inline-block">Student</span>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3 text-sm">
          {[["CMS ID", student?.cms_id],["Room No.", student?.room_no],["Department", student?.dept],["Course", student?.course],["Batch", student?.batch],["Contact", student?.contact]].map(([label, val]) => (
